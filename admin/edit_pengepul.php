<?php
    ...
    if (isset($_POST['register'])) {
        ...
        $sql = mysqli_query($conn, "SELECT kab_kota from pengepul where username = '$user1'");
        ...
        $result = mysqli_query($conn, "SELECT username from pengepul where username = '$username1' and username != '$user1'");

        $result2 = mysqli_query($conn, "SELECT email from pengepul where username = '$user1'");
        $result3 = mysqli_fetch_array($result2);
        $email1 = $result3['email'];
        $result4 = mysqli_query($conn, "SELECT email from pengepul where email = '$email1' and email != '$email'");

        //cek username sudah ada atau belum
        if (mysqli_fetch_assoc($result)) {
            echo "<script>alert('Username tidak tersedia');
            window.location.href = 'edit_pengepul.php';
            </script>";
            return false;
        }
        ...
        //update ke database
            ...
            if($filename != ""){
                ...
                //update tanpa foto
                mysqli_query($conn, "UPDATE pengepul set username='$username1', nama='$nama', email='$email', password='$password', 
...
<head>
    ...
    <title>Tresaikel - Edit Pengepul</title>
</head>
...
<body style="background-image: url('../img/jojo.jpg'); background-size: 100%;">   
    ...
    <p>(Admin)</p>
       ...
       <?php
            ...
            $sql = "SELECT * FROM pengepul WHERE username='$user1'";
       ...
       <li class="active"><a href="pengepul.php">
